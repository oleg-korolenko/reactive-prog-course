...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
    ...
    /** Given a list of futures `fs`, returns the future holding the list of values of all the futures from `fs`.
      ...
      */
    def all[T](fs: List[Future[T]]): Future[List[T]] = seq(fs)
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      for (f <- fs) f.onComplete(p.tryComplete(_))
      p.future
    }
  ...
  def seq[T](fs: List[Future[T]]): Future[List[T]] = async {
    var tempFs = fs
    val result = ListBuffer[T]()
    while (!tempFs.isEmpty) {
      result += await {
        tempFs.head
      }
      tempFs = tempFs.tail
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    /** Returns the result of this future if it is completed now.
      ...
      */
    def now: T = if (f.isCompleted) Await.result(f, Duration(0, SECONDS)) else throw new NoSuchElementException
    ...
    /** Continues the computation of this future by taking the result
      ...
      * The resulting future contains a value returned by `cont`.
      */
    def continue[S](cont: Try[T] => S): Future[S] = f.map(res => cont(Try(res)))
