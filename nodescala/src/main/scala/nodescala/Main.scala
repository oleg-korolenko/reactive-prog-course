...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val listener = myServer.createListener("/test")
    val myServerSubscription = listener.start()

    // TO IMPLEMENT
    // 2. create a future that expects some user input `x`
    //    and continues with a `"You entered... " + x` message
    val userInterrupted: Future[String] = Future.userInput("Your entered ")

    // TO IMPLEMENT
    // 3. create a future that completes after 20 seconds
    //    and continues with a `"Server timeout!"` message
    val timeOut: Future[String] = {
      Future.delay(Duration(20,SECONDS)).flatMap(_=> Future.successful("Server timeout!"))
    }
